...
- For each task: build implementer prompt from tg context and `.cursor/agents/implementer.md`; dispatch implementer (Task tool, agent CLI, or mcp_task per subagent-dispatch). Use `model="fast"` when dispatching implementers and documenters; omit `model` for reviewers, analysts, and investigators (they inherit the session model). After implementer completes, dispatch reviewer with task context + diff; if FAIL, re-dispatch implementer once; after 2 failures, do that task yourself or escalate to the **fixer** (see Escalation below).
