...
function getOneContext(
  taskId: string,
  config: Config,
): ResultAsync<ContextOutput, AppError> {
  const q = query(config.doltRepoPath);
  return resolveTaskId(taskId, config.doltRepoPath).andThen((resolved) =>
    q
      .select<{
        task_id: string;
        title: string;
...
function printOneContext(d: ContextOutput): void {
  ...
  if (d.risks != null && Array.isArray(d.risks) && d.risks.length > 0) {
    ...
    d.risks.forEach(
      (r: { description?: string; severity?: string; mitigation?: string }) => {
        console.log(
            ...
            if (isJson) {
              console.log(
                JSON.stringify(
                  items.map((d) => ({
                    ...d,
                    token_estimate: estimateJsonTokens(d),
                  })),
                  null,
                  2,
                ),
