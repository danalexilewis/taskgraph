...
Reference document for structured risk assessment of code changes and proposed features. Use this when applying the risk skill or when evaluating risk manually.
...
1. **Identify scope** — What is being changed? (plans, tasks, files, domains.) Use `tg crossplan summary --json` when available; otherwise use `tg status --tasks`, `tg next`, and plan files to get plans, tasks, file_tree overlaps, and domains.
