 ...
 */
describe.serial(
  "Plan-level worktree creation, reuse, commit flow, and plan-branch merge",
  () => {
...
`;
    ...
    it("(5) tg done --merge on a second task: plan branch has both task commits", async () => {
      ...
      expect(logOut.stdout).toContain("task 2 work");
      expect(fs.existsSync(path.join(planWtPath5, "task2-file.txt"))).toBe(
        true,
      );
      expect(fs.existsSync(path.join(planWtPath5, "task3-file.txt"))).toBe(
        true,
      );
    }, 30000);
...
`;
      ...
      await execa(
        ...
        [
          ...
          "remove",
          "--force",
          `.taskgraph/worktrees/${raceTaskIdA}`,
        ],
        { cwd: tempDir },
      ).catch(() => {});
      await execa(
        ...
        [
          ...
          "remove",
          "--force",
          `.taskgraph/worktrees/${raceTaskIdB}`,
        ],
        { cwd: tempDir },
      ).catch(() => {});
    }, 30000);
