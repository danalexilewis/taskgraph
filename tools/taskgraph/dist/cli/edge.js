        ...
        const result = await (0, utils_1.readConfig)().asyncAndThen((config) => {
            if (type === "blocks") {
                return neverthrow_1.ResultAsync.fromPromise((async () => {
                    const existingEdgesQuery = `SELECT from_task_id, to_task_id, type FROM edge WHERE type = 'blocks';`;
                    const existingEdgesResult = await (0, connection_1.doltSql)(existingEdgesQuery, config.doltRepoPath);
              ...
              VALUES (
                '${(0, escape_1.sqlEscape)(fromTaskId)}',
                '${(0, escape_1.sqlEscape)(toTaskId)}',
                '${(0, escape_1.sqlEscape)(type)}',
                ${options.reason ? `'${(0, escape_1.sqlEscape)(options.reason)}'` : "NULL"}
              );
            `;
                    return (0, connection_1.doltSql)(insertEdgeSql, config.doltRepoPath)
