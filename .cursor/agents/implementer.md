...
- **[2026-03-02]** Resolving merge conflicts in code: do not take "ours" or "theirs" wholesale when both sides introduced different correctness fixes (e.g. one added `await`, the other changed `process.exit(0)` to `process.exitCode = 0`). Keep both fixes; verify the merged result compiles and preserves intended behavior from each branch.
- **[2026-03-02]** Dashboard/TUI: same section data (row limits, sorted plans, table data) computed in multiple places caused duplication and drift. Compute dashboard section data in one helper and pass into build/update functions.
- **[2026-03-02]** User-supplied filters in SQL (e.g. plan_id, domain) built with string concat and sqlEscape. Prefer parameterized queries where the API supports them; use sqlEscape only when it does not.
- **[2026-03-02]** Functions that return `T | null` for "unavailable" (e.g. runWithServerConnection): document null and require callers to handle it, or return Result and treat unavailability as an explicit error path.
