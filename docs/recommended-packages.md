...
| CLI spinner   | ora                          | `ora`                   | Terminal spinner + elapsed-time feedback for long-running commands; use with a wrapper script (e.g. `scripts/run-with-feedback.ts`) to show dynamic “Running … Ns” and “Finished in Ns”.                          |
