...
| `tg context --hive [--json]`                           | HiveSnapshot of all doing tasks (agents, phases, files, recent notes); use for hive coordination |
...
3. **Reflect and give back** — Given my local context (files I’m editing, discoveries, blockers), should any _other_ task be updated? If yes, run `tg note <otherTaskId> --msg "..."` so that task’s context (and future hive readers) see it.

So: **read group → reflect on self → optionally update other tasks’ notes.** Notes are the only way to push context into another task’s view; the hive snapshot is the way to pull. See [agent-strategy.md](agent-strategy.md#hive-coordination-context-ping-as-impetus) and [.cursor/agent-utility-belt.md](../.cursor/agent-utility-belt.md) for the pattern and for shared learnings as agents experiment.

## Formation and Slots

When multiple `/work` instances run, they coordinate via a **formation** defined in the **situation report (sitrep)**. The sitrep (e.g. `reports/sitrep-YYYY-MM-DD-HHmm.md`) includes a **Formation** section that recommends which lead roles to fill and how many of each.

### Sitrep breadcrumb

Multiple `/work` instances **check the breadcrumb first** (before generating a sitrep). The breadcrumb file is **`.taskgraph/sitrep-breadcrumb.json`** (repo root; git-ignored). One agent may claim `"making_sitrep"`; others that see a recent claim (within 10 minutes) **skip sitrep generation** and go straight to task pull or read the existing sitrep. When returning from work (e.g. after completing tasks), agents **re-check the sitrep**; if it is **stale (older than 30 min)** they may refresh it. This way agents cycle in and out of coordination by re-reading the sitrep when they come back. Full format, staleness rules (10 min for `making_sitrep`, 30 min for sitrep), and edge cases are in [docs/leads/sitrep-breadcrumb.md](leads/sitrep-breadcrumb.md).

- **Formation** — The recommended set of roles (e.g. execution-lead, overseer, investigator-lead, planner-lead) with cardinality (min–max per role). The sitrep is generated by a sitrep-analyst or reused if not stale (staleness: 30 min; coordination via sitrep breadcrumb).
- **Slots** — Each role has a number of "slots" (e.g. overseer has 0–1, execution-lead has 1–N). A `/work` instance **self-selects** by reading the sitrep and `tg status --tasks`, then claiming the highest-priority unfilled role. Slot claiming is **advisory** (no lock); the human avoids over-spawning by how many `/work` invocations they start.
- **Human controls cluster size** — The human decides how many leads to run. Each instance reads the sitrep, sees which roles are already filled (from doing tasks and agent names), and picks an available slot. Relationship to existing coordination: `tg status` "Active work" shows doing tasks and owners; the formation adds a recommended shape so instances can choose a role consistently.

See [docs/leads/README.md](leads/README.md) (Sitrep and Formation) and [docs/agent-strategy.md](agent-strategy.md#micro-cluster-model).
