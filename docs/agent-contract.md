...
tg show <taskId>
    ...
    -   `intent`: The goal or purpose of the task.
    -   `scope_in`: What is explicitly part of this task.
...
tg done <taskId> --evidence "<text>" [--checks <json>]
    ...
    -   Summaries of command outputs.
    -   Relevant Git commit hash(es) if code changes were made.
    -   Optionally, `--checks` can be used to report on specific acceptance criteria met.
