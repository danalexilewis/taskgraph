...
| `priority`      | `INT`          | `DEFAULT 0`       | Queue priority: 1 = most important (first in status/dashboard/next), lower number = higher in queue                                                                                                                                                                                                                                                                                                                            |
...
| `body`       | `JSON`     | `NOT NULL`                       | JSON payload. Conventions: `started` → `{ agent, timestamp }`; `note` → `{ message, agent, timestamp }` (for review verdicts, message is JSON `{ type, verdict, reviewer, stage }` — see [multi-agent](multi-agent.md)); `done` → may include evidence (e.g. gate or test result). |
